...
## Blockers
- **Deployment URL (VERIFIED 2026-05-31):** `t_f0b63354` recovered public Vercel access by deploying the real Next.js app from `smart-link-app/` into a clean `smart-link-app` Vercel project and moving the vanity alias. Verified unauthenticated HTTP 200 for `https://smart-link-mu.vercel.app/`, `https://smart-link-o0nzgx2of-ihorbohdanov-5540s-projects.vercel.app/`, and `https://smart-link-app-swart.vercel.app/`. Follow-up live production QA now lives in `t_4b4b31e0`; docs update is gated behind it in `t_e30891d1`.
...
1. **Approve/send first pilot outreach touch** → verify Gerritsen Barber Shop & Hair Salon's Instagram/email contact, then send the first prepared opener from `docs/pilot-outreach.md`.
2. **Keep public URL claims honest** → use the verified Vercel public URL from the deployment blocker notes, but do not claim a custom domain or fully live owner reporting until QA/docs cards confirm it.
